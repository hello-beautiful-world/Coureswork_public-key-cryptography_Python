...
返回值:fast_c——明文对应密文
'''
def RSA_encryption(n,e):
    m1=input("请输入明文:")
    m=int(m1)
    
    for _ in range(200):
        fast_c=fast_mod(m,e,n)
    print(f"加密明文{m1}所得密文为{fast_c}")
    return fast_c
...
def RSA_decryption(c,d,p,q,n):
    ...
    h = (q_inv * (m_p- m_q)) % p
    plaintext = (m_p+h * q)%n
    print(f"解密明文{c}所得明文为{plaintext}")
    return plaintext 
    
p,q,n,e,d=key_generation(5)   
ciphertext1=RSA_encryption(n,e)
RSA_decryption(ciphertext1,d,p,q,n)
