...
void main()
{
	...
	scanf("%d %d %d",&x1,&y1,&n);
	EDPC(x1,y1,n,sum);
	printf("%d(%d,%d)=(%d,%d)",n,x1,y1,sum[0],sum[1]);
}
//取模，a mod p 
...
int createGroud()//椭圆曲线群的构造                  //x[maxsize],y[maxsize];p,a,b,num;是全局变量 
{
	...
	for(i=0;i<p;i++)
	{
		...
		if(fabs(quickPow(fmod(pow(i,3)+a*i+b,p),(p-1)/2,p)-1)<0.0000000000001)//算出勒让德符号，判断是否有二次剩余，若是将i存入横坐标数组 
		{
