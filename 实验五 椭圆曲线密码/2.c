...
int createGroud()//椭圆曲线群的构造                  //x[maxsize],y[maxsize];p,a,b,num;是全局变量 
{
	int j=0,i,k;
	for(i=0;i<p;i++)
	{
		...
		if(fabs(quickPow(fmod(pow(i,3)+a*i+b,p),(p-1)/2,p)-1)<0.00000001)//算出勒让德符号，判断是否有二次剩余，若是将i存入横坐标数组 
		{
...
{
	int P[2],Q[2],z,n1=-1,n2=-1,i;
	while(1)//输入坐标并保证输入的坐标在椭圆曲线上
	{
		printf("\n请输入两个做加法运算的坐标：\n");
		scanf("%d %d %d %d",&P[0],&P[1],&Q[0],&Q[1]);
		for(i=0;i<num;i++)
		{
			if(P[0]==x[i]) n1=i;//循环结束时n1为最后一个横坐标为x[i] 的点的下标 
			if(Q[0]==x[i]) n2=i;
		}
		if((P[1]!=y[n1]&&P[1]!=y[n1-1])||(Q[1]!=y[n2]&&Q[1]!=y[n2-1])||n1==-1||n2==-1)//注意！！！有横坐标相等的点，一个横坐标下标可能对应两个纵坐标 
		{
			printf("您输入的坐标不在椭圆曲线上！！！"); 
			continue;
		}
		break;
	}
	...
	if(P[0]==Q[0]&&(P[1]+Q[1])%p==0)return; //P为Q的加法逆元，相加为无穷远点，设定为（0，0） 
	if(P[0]!=Q[0]||P[1]!=Q[1])//Q和P不相等 
		z=fmod2((Q[1]-P[1])*extended_euclidean(p,Q[0]-P[0]),p);
	else z=fmod2((3*P[0]*P[0]+a)*extended_euclidean(p,2*P[1]),p);//Q和P相等
	add[0]=fmod2(z*z-P[0]-Q[0],p);
	add[1]=fmod2(z*(P[0]-add[0])-P[1],p);
	printf("(%d,%d)+(%d,%d)=(%d,%d)",P[0],P[1],Q[0],Q[1],add[0],add[1]);
	return;
}
